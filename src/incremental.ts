 ...
 *
 * Manifest 存储位置：{cwd}/.forge-loop/cache/{platform}.json
 * 格式：{ "agents/orchestrator.md": { "hash": "sha256..." } }
 *
 * 路径选择：放在 .forge-loop/cache/ 子目录，与用户领域文件（.opencode/domains/）物理隔离，
 * 避免团队共享的领域 JSON 与本机 manifest 缓存混放。整个 .forge-loop/cache/ 应被 gitignore。
 *
 * 工作流程：
...
const MANIFEST_DIR = ".forge-loop/cache";
