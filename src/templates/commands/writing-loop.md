...
2. 若状态文件存在且未完成，询问恢复还是新建：
   ...
   - 新建：删除状态文件，从零开始。
3. 把请求拆解为 TaskList（每项含 id / title / accept_criteria / depends_on / target_docs）。
...
7. 初始化 `consecutive_failures = 0`。

## 执行路径

每轮：
1. 从 TaskList 选下一个可执行任务（pending + 依赖已 done）。
2. 委派给 writing-author，注入 `当前任务` + `写作边界` + `术语表` + `验证命令`。
3. 执行者产出后，委派 writing-reviewer 复核。
