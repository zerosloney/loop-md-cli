...
| `graph`   | graph-orchestrator / graph-worker / graph-reviewer | ralph-graph        | **图路由范式**：DAG 路由 + 激活集执行。默认动态模式（AI 运行时分解）；静态用 `--tasks-file` |
...
内置 `graph` 领域默认走**动态模式**——生成的命令指示主控 Agent 在运行时从 `$ARGUMENTS` 自行分解 DAG（零配置）。想要**静态**路由表，用 `--tasks-file`：

```bash
# 动态（默认）：AI 运行时分解任务
loop-md-cli --kilo --domain graph

# 静态：从 JSON 文件注入项目专属 DAG
loop-md-cli --kilo --domain graph --tasks-file ./my-tasks.json
```

tasks 文件是 JSON 数组（`[{id, title, depends_on, accept_criteria}]`）或 `{tasks: [...]}`，CLI 计算拓扑排序并注入路由表。下方示例展示如何定义**自定义** graph 领域及自己的任务 DAG（团队共享）：
...
| `--tasks-file`         | `-t`        | 外部 tasks DAG 文件（JSON），为 graph 注入静态路由表              |
